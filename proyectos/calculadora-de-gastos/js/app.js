  ...
  function delMov(id){
    const item = movs.find(m=>m.id===id);
    if (!item) return;
    if (!confirm(`¿Eliminar "${item.d}" por ${fmt(item.v)}?`)) return;
    movs=movs.filter(m=>m.id!==id); scheduleSave(); renderP();
    if (document.getElementById('panel-resumen').classList.contains('active')) renderResumen();
  }
  ...
  function renderP(){
    ...
    const buscarEl = document.getElementById('pBuscar');
    const q = buscarEl ? buscarEl.value.trim().toLowerCase() : '';
    let fil = filtP==='todos' ? movs : movs.filter(m=>m.t===filtP);
    if (q) {
      fil = fil.filter(m => (m.d||'').toLowerCase().includes(q) || (m.c||'').toLowerCase().includes(q));
    }
  ...
  function exportarBackup() {
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = `backup_calculadora_${new Date().toISOString().slice(0,10)}.json`;
    document.body.appendChild(a);
    a.click();
    a.remove();
    URL.revokeObjectURL(url);
    ...
    input.onchange = function(e) {
      ...
      reader.onload = function(ev) {
        try {
          ...
          if (!data.movimientos && !data.integrantes) {
            alert('❌ El archivo no parece ser un backup valido de la calculadora.');
            return;
          }
          if (!confirm('⚠️ Esto REEMPLAZARA todos tus datos actuales con los del backup.\n\n¿Continuar?')) return;
          ints = data.integrantes || [];
          movs = data.movimientos || [];
          budgetLimit = data.budget_limit || 0;
          metasData = data.metas || { emergencia_total: 0, ahorro_asignaciones: [], gustos_asignaciones: [] };
          ...
          scheduleSave();
          renderInts(); renderGass(); renderP();
          if (document.getElementById('panel-metas').classList.contains('active')) renderMetasPanel();
          if (document.getElementById('panel-resumen').classList.contains('active')) renderResumen();
          alert('✅ Backup restaurado correctamente.\n\nSe cargaron ' + movs.length + ' movimientos y ' + ints.length + ' integrantes.');
        } catch (err) {
